...
export const generateComponentContext = async (prompt) => {
    let data = await callSpark(sparkConfig.projectIDs.generateComponentContext, prompt)
    return JSON.parse(data[0].output)
}

export const mapComponentsToAPIs = async (prompt) => {
    let data = await callSpark(sparkConfig.projectIDs.mapComponentsToAPIs, prompt)
    return JSON.parse(data[0].output)
}